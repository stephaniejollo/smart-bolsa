...
try:
    import numpy as np
    import pandas as pd
    from .data import load_builtin_sample, split_walk_forward
    from .env import MultiAssetEnv
    from .agent import DoubleQAgent, AgentConfig
...
- **Selecionar menos ações** (por exemplo, 3–6) para reduzir trocas.  
...
- **Rever a seleção de ações** (menos papéis pode reduzir custos de troca).  
...
""")
    ...
    # preenche início/fim para cobrir a janela inteira
    if len(series_all):
        if series_all.index[0] > s0:
            pad_idx_left = pd.bdate_range(s0, series_all.index[0] - pd.Timedelta(days=1))
            if len(pad_idx_left):
                series_all = pd.concat([pd.Series(series_all.iloc[0], index=pad_idx_left), series_all])
        if series_all.index[-1] < s1:
    ...
    if series_all.empty:
        ...
        ax.axvline(s0, linestyle="--", linewidth=1.2)
        ax.axvline(s1, linestyle="--", linewidth=1.2)
        ax.set_xlim(s0, s1)
